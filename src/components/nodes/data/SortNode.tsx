...
interface SortNodeProps extends Omit<React.ComponentProps<typeof BaseNode>, 'children'> {
...
const ComparatorDisplay = styled.div`
...
export const SortNode: React.FC<SortNodeProps> = ({
  ...
  const { inputArray, comparator, sortOptions, outputArray } = useMemo(() => {
    const arrayPort = data.inputs.find(port => port.label === 'array');
    const comparatorPort = data.inputs.find(port => port.label === 'comparator');
    ...
    return {
      ...
      comparator: typeof getPortValue(comparatorPort) === 'string' ? getPortValue(comparatorPort) : '(a, b) => a - b',
      sortOptions: getPortValue(optionsPort) || { order: 'asc', stable: true },
      ...
      <NodeContainer $isProcessing={isProcessing}>
        ...
        <NodeContent>
          ...
          <SortOptions>
            ...
            Stable: {sortOptions.stable ? 'true' : 'false'}
          </SortOptions>
          
          <ArrayVisualizer type="output">
            Sorted: [{outputArray.slice(0, 3).join(', ')}
