...
| CL-010 | rust | P0 | `complex/rust` | Inheritance is `base: Machine` composition, but bodies use `self.Power` / `Sensor::new()` without field projection or `new` impl — will not compile as Rust. Needs inheritance-lowering plan. | needs-system-plan (transpiler IR / Rust lowering) | open |
...
| CL-014 | verse | P0 | `simple/verse`, `complex/verse` | Get User Input emits `var _vvs_input_… : float = 0.0` stub — not string input. Locked fidelity: behavioral nodes must map to real locatable constructs. | needs-system-plan (Verse GetInput / packs) | open |
| CL-015 | verse | P0 | `complex/verse` | For-each over array emits invalid hybrid `for (float val : Readings) {` (C++-like braces, unclosed block) instead of Verse `for (val : Readings):`. | needs-system-plan (Verse for-loop pack / lowering) | open |
