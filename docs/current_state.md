...
Shipped (August 2026): C++/call-site overload emit, language profile JSON packs, off-thread transpile worker, rust console env + optional `devcontainer` ref (no Docker runtime), Library client token search. Community catalog still not a product. Backstage `template.yaml`/skeleton import + Nunjucks `{moduleName}` normalize; environment template refresh (`applied` / `kept-yours` / `already-current`; full merge TBD); host skip/emit + custom path + `appliedTemplate` (in-editor host edit still planned). In-app roadmap: `graph-doc-split` shipped (in-memory per-tab documents; folder save one `.graph.json` per container/function; localStorage still full snapshot); `mobile` partial (Agent/Bot/StatusBar chip hidden at max-width 768px; coarse pin snap 40px vs mouse 20px; gestures and radial menus still planned). `extends-list-mi-locked-visual`, `yield-statement-later`, `switch-match-cl017`, `env-typespec-emitter` shipped (python/cpp multi-base emit; `yield_stmt` py/gd; Switch native match; TypeSpec CLI → apiSurface).
...
Shell and core interactions are in place. **UI backlog:** [`.agents/memory/incomplete-ui.md`](../.agents/memory/incomplete-ui.md) — **U84–U92, U94–U99, U101–U102, U104–U119 shipped** (August 2026); Function constructor/destructor role + leftover-role locks + settings search audit shipped. Remaining: CL-014 honest (x), U93 long-term, U90/library Phase 3. U100 cut. **U103 locked** as Class (field or Extends; no Component node) — not remaining work. **Extends list UI shipped** (python/cpp print every Extends row; js/gd/verse/cs first parent only; go/rust hidden). `lambda_define`, `flow_try`, `yield_stmt`, and Switch match (CL-017) shipped.
...
| Extends list (MI visual) | **Partial** — list UI on Declare Class (`extendsTypes`, `[0]===extendsType`); python/cpp print every Extends row; js/gd/verse/cs first parent only (C# extras are Implements, not built); go/rust list hidden; Super still first parent |
