...
| Yield (`yield_stmt`) | Done — spawn python/gdscript; hidden elsewhere; Python `yield` / bare `yield`; no invented generator syntax |
| Switch match (CL-017) | Done — Python `match` / Rust `match`; C#/JS/C++ keep `switch`; GDScript/Go/Verse keep if-cascade |
