...
export function FloatingPanelShell({
  ...
  return (
    <GraphWheelShield
      className={`absolute ${CORNER_CLASS[corner]} z-[45] ${widthClass} ${
        expanded ? maxHeightClass : 'max-h-[148px]'
      } flex flex-col bg-zinc-950/96 border border-zinc-800 rounded-md overflow-hidden pointer-events-auto`}
    >
      ...
      <div className="flex-1 overflow-y-auto px-2 py-1.5 min-h-0">{children}</div>
    </GraphWheelShield>
  );
}
