...
export function LibraryView({ browseMode = false }: { browseMode?: boolean } = {}) {
  ...
  const { repos: gitRepos, catalogs: gitCatalogs, loading: gitLoading, error: gitError, addCatalogRepo, removeCatalogRepo } = useGitCatalog();
  ...
  const librarySections = (
    [
      ...
      { id: 'git_catalogs' as const, label: 'Git catalogs', icon: GitBranch, count: gitRepos.length },
      ...(!browseMode
        ? [
            {
  ...
  const sectionMeta = {
    ...
    git_catalogs: {
      title: 'Git catalogs',
      description: 'Browse public GitHub catalog indexes and open their source repositories.',
      icon: GitBranch,
    },
    installed: {
      title: 'Installed',
  ...
  return (
    <div className="flex h-full w-full bg-zinc-950 overflow-hidden text-zinc-300 font-sans">
      ...
      <div className="flex-1 flex min-w-0 h-full">
        <div className="flex-1 flex flex-col h-full bg-zinc-950 overflow-y-auto px-4 py-6 sm:px-8 sm:py-8 min-w-0">
          <div className="max-w-4xl mx-auto w-full">
            <div className="sm:hidden flex gap-2 overflow-x-auto mb-5" aria-label="Library sections">
              {librarySections.map(({ id, label }) => (
                <button key={id} type="button" onClick={() => setActiveSection(id)}
                  className={`shrink-0 rounded border px-3 py-2 text-xs ${section === id ? 'border-indigo-500 text-indigo-200' : 'border-zinc-800 text-zinc-400'}`}>{label}</button>
              ))}
            </div>
            ...
            {section === 'git_catalogs' && (
              <div className="space-y-6">
                {gitError ? <p role="alert" className="text-xs text-amber-400">{gitError}</p> : null}
                {gitLoading ? <p className="text-xs text-zinc-500">Loading public catalogs…</p> : null}
                      ...
                      <div className="space-y-2">
                        ...
                        {gitCatalogs[repo.id]?.assets.filter((asset) =>
                          ...
                          <a key={asset.id} href={asset.repoUrl} target="_blank" rel="noreferrer"
                            ...
                            <span className="block mt-1 text-xs text-zinc-500">{asset.description}</span>
                          </a>
                        ))}
                      </div>
