...
export function useGraphTabSync({
  ...
  const getTabMeta = useCallback(
    (tabId: string): GraphTabMetadata => {
      ...
      return doc?.metadata ?? defaultTabMetadata(tabMeta?.type ?? 'function', tabMeta?.name ?? 'Graph');
    },
    ...
    const metadata =
      tabId === 'main'
        ...
        : documentsRef.current.get(tabId)?.metadata ??
          defaultTabMetadata(tabMeta?.type ?? 'function', tabMeta?.name ?? 'Graph');
  ...
  const updateActiveTabMetadata = useCallback(
    (patch: Partial<GraphTabMetadata>) => {
      ...
      const base = doc.metadata ?? defaultTabMetadata(tabMeta?.type ?? 'function', tabMeta?.name ?? 'Graph');
  ...
  useEffect(() => {
    ...
    const tabMeta = openTabs.find((t) => t.id === activeGraphTab);
    const tabType = tabMeta?.type ?? 'function';
